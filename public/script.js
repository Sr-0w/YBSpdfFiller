...
document.getElementById('securitasForm').addEventListener('submit', function(event) {
    ...
    // Translate component names to the corresponding PDF field names
    const componentsListItems = addedComponentsList.querySelectorAll('li');
    componentsListItems.forEach(item => {
        const componentName = item.textContent.trim();
        const mappedName = componentsData[componentName];
        if (mappedName) {
            const textInput = item.querySelector('input[type="text"]');
            formData.set(mappedName, textInput.value);
        }
    });
