...
document.getElementById('securitasForm').addEventListener('submit', function(event) {
    ...
    // Collect the components data
    const addedComponents = addedComponentsList.getElementsByTagName('li');
    for (let i = 0; i < addedComponents.length; i++) {
        const component = addedComponents[i].textContent.trim();
        if (formData.has(component)) {
            const existingValue = parseInt(formData.get(component), 10);
            formData.set(component, existingValue + 1);
        } else {
            formData.set(component, 1);
        }
    }
